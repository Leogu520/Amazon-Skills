...
把标题作为商品身份字段，而不是关键词容器。保留防止买错的事实，把次要信息分配到合适字段，并对每个候选标题执行校验。内置校验器只对英文标题提供确定性的语义检测；其他语言标题统一转人工审核。
...
- 把 `verified_claims` 当作精确的证据引用。只能使用规范化声明 ID 或文档列出的精确别名，不得用 `claim`、`tested`、`verified` 等宽泛字符串消除警告。
...
内置政策环境只对 `US`/`en_US` 和 `UK`/`en_GB` 做确定性匹配。字段缺失、其他站点语言组合和非英文标题仍执行 75 字符后备检查，但必须返回 `MANUAL_REVIEW`。不得宣称英文促销词、重复词、兼容关系和声明检测已经完整覆盖不支持的语言。
...
不得只信任 `media_exempt`。只有 Product Type 属于内置识别的媒介类型（`ABIS_BOOK`、`BOOK`、`BOOKS` 或 `DVD`），并且完整站点政策环境匹配时，才能在本地跳过长度限制；否则继续执行 75 字符后备检查并返回 `MANUAL_REVIEW`。
...
对每个最终候选标题运行 `scripts/validate_titles.py`。修复全部错误后再推荐；所有警告、`risk_codes` 和 `review_reasons` 必须解决或明确展示。
...
`recommended_title`、`char_count`、`item_highlights`、`item_highlights_char_count`、`retained_terms`、`moved_terms`、`deleted_terms`、`schema_version`、`status`、`errors`、`warnings`、`review_status`、`review_reasons`、`risk_codes` 和 `policy_context`。

校验器保留旧的 `status`、`errors` 和 `warnings`；发布门禁以 `review_status` 为准。除非用户明确要求且命令中提供 `--allow-overwrite`，不得覆盖批量输入源文件。

## 审核状态规则

- `READY`：硬性校验全部通过，必要事实完整，没有未解决的证据或关系风险。
- `READY_WITH_WARNINGS`：硬性校验通过，但仍有不阻断发布的风格或数据质量警告。
- `MANUAL_REVIEW`：存在任何确定性错误、缺少防误购事实或政策环境字段、敏感声明未验证、兼容或商标关系含糊、父子体不确定、标题语言不支持、媒介豁免未验证、合规风险或来源数据冲突。
